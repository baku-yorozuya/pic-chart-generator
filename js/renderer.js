...
class ChartRenderer {
  ...
  _drawSegment(item, index, offset, onClick) {
    ...
    circle.setAttribute(
      "stroke-dasharray",
      `${dash} ${this.config.circumference}`
    );
    circle.setAttribute(
      "stroke-dashoffset",
      -(offset * this.config.circumference) / 100
    );
  ...
  _drawLabel(item, index, offset, labelPositions, bounds) {
    const angle =
      (offset + item.percent / 2) * 3.6 * (Math.PI / 180) - Math.PI / 2;
    const isRight = Math.cos(angle) > 0;

    let y_end =
      this.config.centerY + Math.sin(angle) * this.config.labelEndRadius;
    ...
    const displayPercent = Number.isInteger(item.percent)
      ? item.percent
  ...
  _adjustViewBox(bounds) {
    ...
    const w = isNaN(bounds.xMax - bounds.xMin)
      ? 200
      : bounds.xMax - bounds.xMin + p * 2;
    const h = isNaN(bounds.yMax - bounds.yMin)
      ? 200
      : bounds.yMax - bounds.yMin + p * 2;
